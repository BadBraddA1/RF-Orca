...
- **Audio Crew mode** — header **Audio Crew** button (desk/tablet only): fullscreen-friendly dense rack; room name + Change room sit in the top bar; phones stay on **List**. **Draw** (FaceTime-style): free ink across the whole gear plane (cells and gaps), with a shared column layout so marks line up on every Live tablet; Clear wipes everyone’s marks; strokes fade after a few seconds.

...
- **Audio Crew mode** — header **Audio Crew** button (desk/tablet only): fullscreen-friendly dense rack; room name + Change room sit in the top bar; phones stay on **List**. **Draw** (FaceTime-style): shared markup locked to the gear matrix so an arrow on CH 05 lands on CH 05 on every Live tablet (same column layout + cell-relative coords); Clear wipes everyone’s marks; strokes fade after a few seconds.

...
- **Audio Crew mode** — header **Audio Crew** button (desk/tablet only): fullscreen-friendly dense rack; room name + Change room sit in the top bar; phones stay on **List**. **Draw** (FaceTime-style): shared markup on every Live crew tablet — toggle Draw, pick a color, scribble over the rack, Clear wipes everyone’s marks; strokes fade after a few seconds.
